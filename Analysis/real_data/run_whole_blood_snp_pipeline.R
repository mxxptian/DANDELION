...
#        p_value
#
#   3. trans_eqtl_file should contain SNP annotation columns required by
#      calc_pair.snp()/med_gene() for SNP-based DANDELION:
#        SNP
#        SNPPos
#        SNPChr
...
trait_code <- get_arg("--trait_code", "GCST90082964")
...
message("DANDELION SNP-based pipeline")
...
get_trait_files <- function(trait_dir, trait_code) {
  files <- list.files(
    trait_dir,
    pattern = paste0("^", trait_code, "_.*\\.[Rr]Data$"),
    full.names = FALSE
  )

  files <- files[
    !grepl("_pair\\.[Rr]Data$|_DACT\\.[Rr]Data$", files)
  ]
...
prepare_trait_pvalues <- function(dat_temp) {
  ...
  # Remove duplicated gene names because gene-level p-value vectors require
  # one p-value per gene identifier.
...
if (length(trait_files) == 0) {
  stop("No trait-specific RData files found in: ", trait_dir, call. = FALSE)
}
if (trait_index > length(trait_files)) {
  stop(
    "trait_index = ", trait_index,
    " exceeds number of trait files = ", length(trait_files),
    call. = FALSE
  )
}

allele_list <- sub(
  paste0("^", trait_code, "_(.*)\\.[Rr]Data$"),
  "\\1",
  trait_files
)
